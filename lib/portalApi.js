...
export const firmApi = {
  ...
  async getEngagement(engagementId) {
    ...
    return { ...mapEngagement(eng), items: (items || []).map(mapItem), myRole, myCanDelete };
  },
  ...
  async uploadSample(engagementId, itemId, file) {
    ...
    const path = `${engagementId}/${itemId}/sample_${Date.now()}_${safe}`;
    const { error: upErr } = await supabase.storage.from("pbc").upload(path, file);
    if (upErr) throw upErr;
    const { error } = await supabase.from("item_files").insert({
      item_id: itemId, engagement_id: engagementId,
      name: file.name, size: file.size, type: file.type, storage_path: path, is_sample: true,
    });
    if (error) throw error;
  },

  async removeSample(fileId, storagePath) {
    await supabase.storage.from("pbc").remove([storagePath]);
...
export const clientApi = {
  ...
  async sampleUrl(token, fileId) {
    const { url } = await callPortal("sample_url", { token, file_id: fileId });
    return url;
  },
};
